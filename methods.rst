...
T-distributed stochastic neighbor embedding (t-SNE) is a non-linear dimensionality reduction method that enables visualization of high-dimensional data in a low-dimensional space, typically 2D or 3D. Unlike linear dimensionality reduction techniques like PCA, T-SNE can effectively separate data that is not linearly separable. Additionally, while PCA tends to preserve the global information in the data, t-SNE tends to preserves the relative distance and the clustering in the data. t-SNE first calculates the distance between every pair of data points. Each data point is then placed within Gaussian distribution while all other data points are distributed according to their distance. Points closer each other are more similar. Point more far away are more diverse each other. A value, called Perplexity, is then calculated to reflect the standard deviation of the data. To finally determine the representation of the data in the low dimensional space,  a t-distribution is used with one degree of freedom. A gradient descent optimization involving an iterative process, is employed to determine the final low-dimensional data representation that reflects the high-dimension accurately. 




t-SNE was computed using
